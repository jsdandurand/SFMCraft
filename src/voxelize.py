...
def create_voxel_grid(pcd, voxel_size=0.05):
    ...
    voxel_coords = np.array([voxel.grid_index for voxel in voxels], dtype=np.float64)
    ...
    return voxel_grid, voxel_coords, voxel_colors

def save_voxel_grid(voxel_grid, output_path):
    """
    Save the voxel grid data for later use.
    """
    # Get voxel data
    voxels = voxel_grid.get_voxels()
    voxel_data = {
        ...
        'colors': np.array([voxel.color if voxel.color is not None else [0.7, 0.7, 0.7] for voxel in voxels])
    }
    
    # Save as numpy file
    np.savez(output_path, **voxel_data)
...
def main():
    ...
    parser.add_argument('--output', type=str, help='Output path for voxel grid data (.npz)')
    ...
    try:
        ...
        # Create voxel grid
        voxel_grid, voxel_coords, voxel_colors = create_voxel_grid(pcd, args.voxel_size)
        
        # Save voxel grid if output path provided
        if args.output:
            save_voxel_grid(voxel_grid, args.output)
