...
def main():
    ...
    sample_cmd = f"python src/sample_video_frames.py --input {args.input} --output {frames_dir} --sample-rate {args.sample_rate}"
    run_command(sample_cmd, "Sampling video frames")

    # Step 2: Run SFM
    sfm_cmd = f"python src/run_sfm.py --input {frames_dir} --output {ply_output}"
